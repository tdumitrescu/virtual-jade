...
function Compiler(node, options) {
  ...
  this.generateLiteral = this.vdom.generateLiteral;
  this.marshalDataset = this.vdom.dataset;
  this.rawProps = this.vdom.rawProps;

  // config overrides
  for (let optionName of [`marshalDataset`, `propsWrapper`, `rawProps`]) {
...
Compiler.prototype.visitLiteral = function(node) {
  ...
  const id = literalCount++;
  return `generateLiteral(${id}, '${escapedSource}')`;
};
...
Compiler.prototype.visitAttributes = function(attrs) {
  ...
  if (this.rawProps) {
    ...
    // easy-peasy, pass through everything directly except class
