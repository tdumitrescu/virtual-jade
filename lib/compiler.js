...
Compiler.prototype.visitLiteral = function(node) {
  ...
  const propsOut = this.wrapProps(`{innerHTML: '${escapedSource}'}`, this.attrWrapper);
...
Compiler.prototype.visitAttributes = function(attrs) {
  ...
  if (this.rawProps) {
    ...
    if (id) {
      const idObj = `{id: ${id}}`;
      const propsObj = props[this.attrWrapper];
      props[this.attrWrapper] = propsObj ? `Object.assign(${idObj}, ${propsObj})` : idObj;
    }
