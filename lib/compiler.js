...
Compiler.prototype.visitAttributes = function (attrs) {
  ...
  // classes are not expected to be an array,
  // so combine them
      ...
      case 'Code':
        ...
        if (/^\s*while/.test(node.val)) {
          buf += `(function(){
            var buf = [];
            ${node.val} {
              buf = buf.concat([${this.visitBlock(node.block, false)}])
            }
            return buf
          }).call(this)`
          break
...
Compiler.prototype.visitCase = function (code) {
  ...
  for (let node of code.block.nodes) {
    if (node.expr === 'default') {
      defaulted = true
      str += `[${this.visitBlock(node.block, false)}]`
      break
    }

    str += `((${code.expr}) == (${node.expr}))
      ? [${this.visitBlock(node.block, false)}]
      : `
  }
...
Compiler.prototype.visitEach = function (code) {
  return `(${code.obj}).map(function (${code.val}, ${code.key}) {
    return [${this.visitBlock(code.block, false)}]
  })`
}
